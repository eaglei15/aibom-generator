...
def normalize_license_id(license_id: str) -> Optional[str]:
    ...
    # Check multi-word / alias mappings first (these can't be fixed up by the library)
    if lower_id in LICENSE_MAPPING:
        return LICENSE_MAPPING[lower_id]

    # Use cyclonedx-python-lib to normalize case ("apache-2.0" → "Apache-2.0")
    fixed = cdx_spdx.fixup_id(license_id)
    if fixed is not None:
        return fixed

    # Unknown but looks like a simple token — return as-is and let the caller decide
